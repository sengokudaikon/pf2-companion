package io.sengokudaikon.isn.compendium.persistence.character.ancestry.repository

import arrow.core.Either
import arrow.core.left
import arrow.core.right
import io.sengokudaikon.isn.compendium.domain.character.ancestry.entity.Ancestry
import io.sengokudaikon.isn.compendium.domain.character.ancestry.entity.AncestryBoost
import io.sengokudaikon.isn.compendium.domain.character.ancestry.entity.AncestryFlaw
import io.sengokudaikon.isn.compendium.domain.character.ancestry.entity.AncestryLanguage
import io.sengokudaikon.isn.compendium.domain.character.ancestry.entity.AncestryTrait
import io.sengokudaikon.isn.compendium.domain.character.ancestry.repository.AncestryRepositoryPort
import io.sengokudaikon.isn.compendium.domain.character.ancestry.repository.VisionSenseRepositoryPort
import io.sengokudaikon.isn.compendium.domain.world.global.repository.LanguageRepositoryPort
import io.sengokudaikon.isn.compendium.domain.world.global.repository.TraitRepositoryPort
import io.sengokudaikon.isn.compendium.enums.toSizeEnum
    ...
    override suspend fun create(command: AncestryCommand): Either<Throwable, Ancestry> {
        ...
        return suspendedTransactionAsync {
            val ancestry = Ancestry.new {
                ...
                rarity = io.sengokudaikon.isn.compendium.enums.Rarity.valueOf(dto.system.rarity.uppercase())
                hp = dto.system.hp
            ...
            val boosts = dto.system.boosts.forEach { (_, value) ->
                if ((value.value.size > 2) || (value.value.isEmpty())) {
                    AncestryBoost.new {
                        this.ancestryID = ancestry
                        this.boost = io.sengokudaikon.isn.compendium.enums.Ability.Anything
                    }
                } else {
                    value.value.map { boost ->
                        AncestryBoost.new {
                            this.ancestryID = ancestry
                            this.boost = io.sengokudaikon.isn.compendium.enums.Ability.valueOf(boost)
                        }
                    }
                }
            }
            ancestry.abilityBoosts.plus(boosts)
            val abilityFlaws = dto.system.flaws.forEach { (_, value) ->
                value.value.map { boost ->
                    AncestryFlaw.new {
                        this.ancestryID = ancestry
                        this.flaw = io.sengokudaikon.isn.compendium.enums.Ability.valueOf(boost)
                    }
                }
            }
