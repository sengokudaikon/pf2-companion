...
class BackgroundRepository(
    ...
    override suspend fun findByName(name: String, criteria: Criteria): Result<BackgroundModel> = super.findByName(name, criteria).map {
        it.feats = findFeats(it).getOrDefault(emptyMap())
        it.actions = findActions(it).getOrDefault(emptyMap())
        it
    }

    override suspend fun findById(id: String, criteria: Criteria): Result<BackgroundModel> = super.findById(id, criteria).map {
        it.feats = findFeats(it).getOrDefault(emptyMap())
        it.actions = findActions(it).getOrDefault(emptyMap())
        it
    }
    ...
    override suspend fun findAll(
        ...
        val backgrounds = super.findAll(page, limit, criteria).getOrDefault(emptyList())
        backgrounds.map {
            it.feats = findFeats(it).getOrDefault(emptyMap())
            it.actions = findActions(it).getOrDefault(emptyMap())
            it
